...
int main(int argc, char ** argv) {
	...
	auto translit = FACTORYCREATENS(Ribbon::Transliteration, Transliterator);
	{
		auto sourceText = u"きょうはいいてんきです";
		auto resultText = translit->SimpleStringConversion(sourceText);
		printf("%s => %s\n", Ribbon::to_utf8(sourceText).c_str(), resultText.u8str().c_str());
	}
	{
		auto sourceText = u"きどうせん";
		auto resultText = translit->SimpleStringPrediction(sourceText);
		printf("%s => %s\n", Ribbon::to_utf8(sourceText).c_str(), resultText.u8str().c_str());
	}
}

std::shared_ptr<Ribbon::IJsonInputModel> s_inputModel;

void EMSCRIPTEN_KEEPALIVE imeInitialize() {
	if (!s_inputModel) { s_inputModel = FACTORYCREATENS(Ribbon, JsonInputModel); }
}

void imeFinalize() { s_inputModel.reset(); }
void imeJsonCommand(const std::string& jsonText) { s_inputModel->JsonCommand(jsonText.c_str()); }
std::string imeCompositionState() { return s_inputModel->CompositionState(); }
std::string imeCandidateState() { return s_inputModel->CandidateState(); }
std::string imeKeyboardState() { return s_inputModel->KeyboardState(); }

EMSCRIPTEN_BINDINGS(my_module) {
  emscripten::function("imeInitialize", &imeInitialize);
  emscripten::function("imeFinalize", &imeFinalize);
  emscripten::function("imeJsonCommand", &imeJsonCommand);
  emscripten::function("imeCompositionState", &imeCompositionState);
  emscripten::function("imeCandidateState", &imeCandidateState);
  emscripten::function("imeKeyboardState", &imeKeyboardState);
}
...
class PlatformWasm :
	public IPlatform,
	public std::enable_shared_from_this<PlatformWasm>
{
private:
...
static struct PlatformWasmRegister {
	PlatformWasmRegister() {
		s_platformHolder = std::make_shared<PlatformWasm>();
		Platform = s_platformHolder.get();
	}
} s_PlatformWasmRegister;
